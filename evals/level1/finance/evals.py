...
finance_dataset = Dataset[Query[ResultT], QueryResult[ResultT]](
    cases=[
        Case(
            name="finance_l1_q1",
            inputs=Query(
                query="What was the total revenue in Q4 2022?", output_type=TotalRevenue
            ),
        ...
        Case(
            name="finance_l1_q5",
            inputs=Query(
                query="What are the top 3 highest expenses in 2023?", output_type=Expenses
            ),
            expected_output=QueryResult(
                result=Expenses(
                    expenses=[
                        ...
                        Expense(
                            expense_id=17, description="Expense 17: Travel", amount=700
                        ),
                    ]
                ),
                tool_calls=[
