from dataclasses import dataclass
from datetime import date

import logfire
from pydantic_evals import Case, Dataset
from pydantic_evals.evaluators import Evaluator, EvaluatorContext

from dream_factory_evals.df_agent import Role, setup_task_and_agent
from dream_factory_evals.hr_1_return_types import (
    DepartmentCount,
    Email,
    Employee,
    Employees,
    Expense,
    Expenses,
    Machine,
    Machines,
    MachineStatus,
    ManagerCount,
    Policies,
    Policy,
    ProductCount,
    ReplacementCount,
    RevenueAmount,
    TotalAmount,
    TotalRevenue,
)

_ = logfire.configure()

HR_RESULT_TYPES = (
    Email
    | DepartmentCount
    | Employee
    | Employees
    | Expense
    | Expenses
    | Machine
    | Machines
    | MachineStatus
    | ManagerCount
    | Policies
    | Policy
    | ProductCount
    | ReplacementCount
    | RevenueAmount
    | TotalAmount
    | TotalRevenue
)


@dataclass
class HRDepartment:
    id: int
    name: str


@dataclass
class HREmployee:
    id: int
    name: str
    department: HRDepartment

    def __str__(self) -> str:
        return (
            f"<employee>\n"
            f"id: {self.id}\n"
            f"name: {self.name}\n"
            f"<department>\n"
            f"id: {self.department.id}\n"
            f"name: {self.department.name}\n"
            f"</department>\n"
            f"</employee>"
        )


@dataclass
class HR:
    employees: list[HREmployee]

    def __str__(self) -> str:
        return f"<employees>\n{'\n\n'.join(str(e) for e in self.employees)}\n</employees>"


@dataclass
class HRQuery:
    query: str
    result_type: type[HR_RESULT_TYPES]


@dataclass
class EvaluateHRQuery(Evaluator[HRQuery, HR_RESULT_TYPES]):
    def evaluate(self, ctx: EvaluatorContext[HRQuery, HR_RESULT_TYPES]) -> bool:
        return ctx.output == ctx.expected_output


hr_dataset = Dataset[HRQuery, HR_RESULT_TYPES](
    cases=[
        Case(
            name="hr_l1_q1",
            inputs=HRQuery(query="What is the email address of Alice Johnson?", result_type=Email),
            expected_output=Email(email="alice.johnson@example.com"),
        ),
        Case(
            name="hr_l1_q2",
            inputs=HRQuery(query="How many departments do we have in the company?", result_type=DepartmentCount),
            expected_output=DepartmentCount(department_count=20),
        ),
        Case(
            name="hr_l1_q3",
            inputs=HRQuery(
                query="List all department policies that were effective from January to June 2023.",
                result_type=Policies,
            ),
            expected_output=Policies(
                policies=[
                    Policy(
                        policy_id=13,
                        title="Support Policy",
                        description="Policy details for Support department.",
                        effective_date=date(2023, 1, 1),
                        department_id=13,
                    ),
                    Policy(
                        policy_id=14,
                        title="Quality Assurance Policy",
                        description="Policy details for Quality Assurance department.",
                        effective_date=date(2023, 2, 1),
                        department_id=14,
                    ),
                    Policy(
                        policy_id=15,
                        title="Logistics Policy",
                        description="Policy details for Logistics department.",
                        effective_date=date(2023, 3, 1),
                        department_id=15,
                    ),
                    Policy(
                        policy_id=16,
                        title="Business Development Policy",
                        description="Policy details for Business Development department.",
                        effective_date=date(2023, 4, 1),
                        department_id=16,
                    ),
                    Policy(
                        policy_id=17,
                        title="Strategy Policy",
                        description="Policy details for Strategy department.",
                        effective_date=date(2023, 5, 1),
                        department_id=17,
                    ),
                    Policy(
                        policy_id=18,
                        title="Compliance Policy",
                        description="Policy details for Compliance department.",
                        effective_date=date(2023, 6, 1),
                        department_id=18,
                    ),
                ]
            ),
        ),
        Case(
            name="hr_l1_q4",
            inputs=HRQuery(query="List all employees who joined in 2023.", result_type=Employees),
            expected_output=Employees(
                employees=[
                    Employee(
                        employee_id=13,
                        first_name="Mia",
                        last_name="White",
                        email="mia.white@example.com",
                        date_joined=date(2023, 1, 15),
                        department_id=13,
                        role="Staff",
                    ),
                    Employee(
                        employee_id=14,
                        first_name="Noah",
                        last_name="Harris",
                        email="noah.harris@example.com",
                        date_joined=date(2023, 2, 10),
                        department_id=14,
                        role="Manager",
                    ),
                    Employee(
                        employee_id=15,
                        first_name="Olivia",
                        last_name="Martin",
                        email="olivia.martin@example.com",
                        date_joined=date(2023, 3, 5),
                        department_id=15,
                        role="Staff",
                    ),
                    Employee(
                        employee_id=16,
                        first_name="Peter",
                        last_name="Thompson",
                        email="peter.thompson@example.com",
                        date_joined=date(2023, 4, 20),
                        department_id=16,
                        role="Staff",
                    ),
                    Employee(
                        employee_id=17,
                        first_name="Quinn",
                        last_name="Garcia",
                        email="quinn.garcia@example.com",
                        date_joined=date(2023, 5, 15),
                        department_id=17,
                        role="Manager",
                    ),
                    Employee(
                        employee_id=18,
                        first_name="Riley",
                        last_name="Martinez",
                        email="riley.martinez@example.com",
                        date_joined=date(2023, 6, 10),
                        department_id=18,
                        role="Staff",
                    ),
                    Employee(
                        employee_id=19,
                        first_name="Sophia",
                        last_name="Robinson",
                        email="sophia.robinson@example.com",
                        date_joined=date(2023, 7, 5),
                        department_id=19,
                        role="Staff",
                    ),
                    Employee(
                        employee_id=20,
                        first_name="Tyler",
                        last_name="Clark",
                        email="tyler.clark@example.com",
                        date_joined=date(2023, 8, 20),
                        department_id=20,
                        role="Manager",
                    ),
                ]
            ),
        ),
        Case(
            name="hr_l1_q5",
            inputs=HRQuery(query="How many managers do we have in the company?", result_type=ManagerCount),
            expected_output=ManagerCount(manager_count=5),
        ),
    ],
    evaluators=[EvaluateHRQuery()],
)


MODEL = "openai:gpt-4o-mini"
USER_ROLE = Role.HR


async def task(inputs: HRQuery) -> HR_RESULT_TYPES:
    task, agent = setup_task_and_agent(main_task=inputs.query, user_role=USER_ROLE, model=MODEL)
    async with agent.run_mcp_servers():
        res = await agent.run(user_prompt=str(task), result_type=inputs.result_type)
        return res.data


report = hr_dataset.evaluate_sync(task=task, name=f"{MODEL.upper()}-{USER_ROLE.value.upper()}-LEVEL-1")
report.print(include_input=True, include_expected_output=True, include_output=True)
